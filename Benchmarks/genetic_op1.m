...
%   ind: is an individual structure.
  ...
  parents = mateselection(index, updateneighbour, 1);
  newpoint = de_crossover([index parents], params.F, params.CR, domain,index);
...
function ind = de_crossover(parents, F, CR, domain,index)
  ...
  hihi=subproblems(index).pbest;
  cross = selectpoints(:,1) + F.*(hihi-selectpoints(:,2));
  ind = selectpoints(:,1);
