function [pareto metric1]= moead_pf( mop, varargin)  %PF based forward march method
...
%         window=5;   
        if mod(itrCounter,window1)==0
            ...
            no=length(archeive_past);
            for i=1:no
                p=archeive_present(:,i);
                tempdist=[];
                for j=1:no
                    q = archeive_past(:,j);
                    r=(p-q).^2;
...
%                     
%                    temp(:,i)=newposition+epsilon*ones(mop.pd,1);
%                 
%                 front(:,i)=evaluate(mop,temp(:,i),itrCounter);
%                 
%              
%             end
%             % checking whether initialisation is good
%                 metric2=igd2(front, mop.name, mop.pd,itrCounter)
%                 if (metric2-metric)<inf;%((((0.1-0.5)*itrCounter)/params.iteration) +0.5)
%                     12
%                     subproblems(i).curpoint.parameter=temp(:,i);
%                     subproblems(i).curpoint.objective=front(:,i);
%                     
%                     disp(sprintf('Itr:%d\tMetric:%1.6f', ...
%         itrCounter, metric2));
%                     itrCounter=itrCounter+1;
...
function update(index, ind, updateneighbour)
  global subproblems idealpoint params window1 step1;
