...
public class Main {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		try{
			...
			while ((line = br.readLine()) != null) {
				...
				double[] object = new double[strObject.length];
		...
		for(int j=0; j<iterations;j++){
			Algorithm a = new Algorithm(30000, 3);
		...
		//System.out.println("SOO - MO-XAB & " +calculateAverage(res.means)+ " (" +calculateStDev(res.means)+ ") & "+calculateAverage(res.cardinalities)+" & " + calculateAverage(res.times) +"\\\\");
	...
	private static void printEP(ArrayList<Cell> bestSolutions) {
		for(Cell cell : bestSolutions){
			//Prints the input of the best solutions
			//System.out.println(bestSolutions.get(i).points.toString());
			//Prints the result with the above input
			String print = "";
			for(double result : cell.getResult()){
